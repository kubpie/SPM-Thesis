...
def rel_SSPvecOrdered(data):
    ...
    for row, depth, ssp, profile in zip(range(len(data[crit_ssp])), data[crit_depths], data[crit_ssp], data['profile']):         
        ...
        for it in range(len(ordered_ssp[:-1])):
            sspval1  =  ordered_ssp[it]
            sspval2 = ordered_ssp[it+1]
            graql_insert_query = f'match $sspval{it} isa SSP_value;'
            graql_insert_query += f' $sspval{it} == {sspval1:.{precision}};'
            graql_insert_query += f' $sspval{it+1} isa SSP_value;'
            graql_insert_query += f' $sspval{it+1} == {sspval2:.{precision}};'
            graql_insert_query += f' insert $ord(deep: $sspval{it}, deeper: $sspval{it+1}) isa SSP-order;'
            graql_queries.append(graql_insert_query)
