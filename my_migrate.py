...
def SSPVec_inner(ssp, dmax, SSP_Input, SSP_Stat, query_type = 'insert'):
    ...
    for md in maxdepth[:maxdepth.index(dmax)+1]:
        #graql_insert_query += ', has depth ' + f'{md}'
        graql_insert_query += ', has SSP_value ' + f'{SSP_Input[ssp].iloc[depth.index(md)]:.{precision}}'
        #graql_insert_query += f', has SSP_grad {Decimal(SSP_Grad[ssp].iloc[depth.index(md)]):.{precision}}'
    graql_insert_query += ';'
...
def rel_Convergence(data):
    ...
    for idx, ray in zip(data.index, data['num_rays']):
        ...
        graql_insert_query += (' insert $conv(converged_scenario: $scn, convergence_minimum: $ray) isa convergence;')
...
def rel_BathySloped(Bathy):
    graql_queries = []
    for index, row in Bathy.iterrows():
    #Bathy-sloped relation
        if row["slope"] != 0:
            graql_insert_query = BottomSegment1_inner(row["bottom_type"], row["d_start"], row["len_flat"], query_type = 'match')
            graql_insert_query += BottomSegment2_inner(row["bottom_type"], row["d_end"], row["len_flat"], query_type = '')
            graql_insert_query += WedgeSegment_inner(row["bottom_type"], row["len_slope"], query_type = '')
            
            graql_insert_query += (' insert $bslp(is_wedge: $ws, start_segment: $bs1, end_segment: $bs2) isa bathy-sloped;')
...
Relations = [
    ...
    #{"NodeName": 'relation: bathy-sloped',
    # "QueryList": rel_BathySloped(Bathy)
    #},
    
    {"NodeName": 'relation: bathymetry', #depends on bathy-sloped
